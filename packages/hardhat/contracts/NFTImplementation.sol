  ...
  Ownable2StepUpgradeable
{
  ...
  function updateUserMilestone(address _participantAddress) external onlyOwner {
    ...
    emit MilestoneUpdated(
      _participantAddress,
      _userMilestone[_participantAddress]
    );
  }
  ...
  function mintNFT(
    ...
    require(
      _nextMilestoneIndex <= totalMilestones,
      'All milestones have been minted'
    );
    ...
    require(
      _userMilestone[_recipient] >= requiredMilestone,
      'User has not met the milestone'
    );
    ...
    _setTokenURI(
      tokenId,
      string(abi.encodePacked(_baseTokenURI, _metadataCID, '.json'))
    );
    _nextMilestoneIndex++;
