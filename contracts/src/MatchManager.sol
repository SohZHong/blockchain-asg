...
contract MatchManager is IEntropyConsumer {
    ...
    function attack(uint256 _battleId) external payable {
        ...
        bytes32 userRandomNumber = keccak256(
            abi.encodePacked(block.timestamp, msg.sender, _battleId)
        );
        ...
        pendingAttacks[sequenceNumber] = PendingAttack(
            _battleId,
            msg.sender,
    ...
    function entropyCallback(
        ...
        Player storage attacker = pending.attacker ==
            battle.player1.playerAddress
            ? battle.player1
            : battle.player2;
        Player storage defender = pending.attacker ==
            battle.player1.playerAddress
            ? battle.player2
            : battle.player1;
        ...
        uint256 damage = attacker.minDamage +
            (uint256(randomNumber) %
                (attacker.maxDamage - attacker.minDamage + 1));
